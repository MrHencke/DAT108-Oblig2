...
public class Oppg3 {
	...
	public static void main(String[] args) {
		List<Ansatt> ansatte = Arrays.asList(new Ansatt("Markus", "Bjerm", Kjonn.MANN, "Sjefutvikler", 900000),
				new Ansatt("Erik", "Herm", Kjonn.MANN, "Frontendutvikler", 480000),
				new Ansatt("Maria", "Johansen", Kjonn.KVINNE, "DevOps utvikler", 450000),
				new Ansatt("Bjørnar", "Hansen", Kjonn.MANN, "Fullstack", 450000),
				new Ansatt("Kjartan", "Aas", Kjonn.MANN, "Konsulent", 450000));
		...
		// 3c
		int gjSnittKvinner = ansatte.stream().filter(a -> a.getKjonn() == Kjonn.KVINNE).map(a -> a.getAarslonn()).mapToInt(b -> b).sum() / antallKvinner;
		...
		for(Ansatt s : sjefer) s.setAarslonn((int) (s.getAarslonn() * 1.07));
		System.out.println("\n3 d)");
		...
		for(int i = 0; i < 1000; i++) heltall.add(i);
